...
exports.getLogin = (req, res) => {
  if (req.user) {//?: when did we set user in html file so that we can access it via req.user?
    return res.redirect('/');
  }
...
exports.postLogin = (req, res, next) => {
  ...
  passport.authenticate('local', (err, user, info) => { //? what does this function set as info
    if (err) { return next(err); }
    if (!user) {
...
###############################*/
  var var_num = 6;
  var result = ['var1', 'var2','var3', 'var4', 'var5', 'var6'][Math.floor(Math.random() * var_num)]
...
exports.postUpdateProfile = (req, res, next) => {
  ...
  User.findById(req.user.id, (err, user) => { //?: where are user and id defined in User..
    if (err) { return next(err); }
    user.email = req.body.email || '';
    user.profile.name = req.body.name || '';
    ...
    user.save((err) => {
      if (err) {
        ...
        return next(err);//?: why do we need to specify next argument in this case?
      }
      req.flash('success', { msg: 'Profile information has been updated.' });
  ...
  User.find().where('active').equals(true).exec(    
    ...
    if (err) {
      ...
      for (var i = users.length - 1; i >= 0; i--) {
        ...
        console.log("Three days is  "+two_days);
        ...
        {
          console.log('just want to know you are working !!');
        }
...
exports.userTestResults = (req, res) => {
  ...
  if (!req.user.isAdmin)//?: how do I know this request has a user item.. or where is this user associated with request ..
  {
    res.redirect('/');
  }
  //we are admin
