 ...
 */
CohortSchema.methods.comparePassword = function comparePassword(candidatePassword, cb) {
 ...
 */
CohortSchema.methods.logUser = function logUser(time, agent, ip) {
...
CohortSchema.methods.logPage = function logPage(time, page) {
...
CohortSchema.methods.logPostStats = function logPage(postID) {
 ...
 */
CohortSchema.methods.getPosts = function getPosts() {
  var temp = [];
 ...
 */
CohortSchema.methods.getPostsAndReplies = function getPostsAndReplies() {
  var temp = [];
  for (var i = 0, len = this.posts.length; i < len; i++) {
...
CohortSchema.methods.getUserPostByID = function(postID) {
...
CohortSchema.methods.getUserReplyByID = function(replyID) {
...
//Return the user reply from its ID
CohortSchema.methods.getActorReplyByID = function(actorReplyID) {
 ...
 */
CohortSchema.methods.gravatar = function gravatar(size) {
  if (!size) {
    size = 200;
  }
  if (!this.email) {
