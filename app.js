...
const logger = require('morgan'); //log data into the console..
 ...
 * Connect to MongoDB.
 */
mongoose.Promise = global.Promise; //?: where did you use this promise?
...
//app.use(compression());
//?: app.use(middleware) is called everytime a request is sent to the server.. 
//before the final request handler is made .. here orders of these app.use middlewares matters
app.use(logger('dev'));
app.use(bodyParser.json()); // to retrive data from the front-end
...
app.use((req, res, next) => {
  ...
  if (!req.user &&
    ...
    console.log("!!!!!!!path is now");
    console.log(req.path);
    req.session.returnTo = req.path;//? what does this one do?
  }
  next();
});
...
function check(req, res, next) { //?: who calls this check function, and where is "next" callback is set?
 ...
 * Primary app routes.
 */
app.get('/', passportConfig.isAuthenticated, scriptController.getScript); //?: these two function require 3 arguments, req, res, next.
//the first two are based on the request, and the result that will be set in the function, but who set the callback or next?
